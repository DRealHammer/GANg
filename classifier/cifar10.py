...
def get_classifier():
    classifier = vgg13_bn(pretrained=False, progress=True, device="cuda")
    classifier.load_state_dict(torch.load('classifier/vgg13_bn.pt'))
    classifier.eval()

    classifier = classifier.to('cuda')

    classifier = nn.Sequential(classifier, nn.Softmax(1))
