...
from models.D2GAN import Discriminator as D2Discriminator

import matplotlib.pyplot as plt

class Discriminator(nn.Module):

    def __init__(self, n_discriminators, img_shape, alpha, beta):
        super(Discriminator, self).__init__()

        self.discriminators = nn.ModuleList([D2Discriminator(img_shape, alpha, beta) for _ in range(n_discriminators)])
        self.n_discriminators = n_discriminators
        self.optimizer = torch.optim.Adam(self.discriminators.parameters(), lr=0.0002, betas=(0.5, 0.999))
        
    def forward(self, x):
        res = torch.zeros(self.n_discriminators, len(x), 2)
        for i, discr in enumerate(self.discriminators):
            d1, d2 = discr.forward(x)
            res[i, :, 0], res[i, :, 1] = d1.view(64), d2.view(64)
        return res
    
    def loss(self, real, fake):
        assert(len(real) == len(fake))
        res = torch.zeros(self.n_discriminators, 2)
        
        for i, discr in enumerate(self.discriminators):
            res[i, 0], res[i, 1] = discr.loss(real, fake)
        return res
...
class GAN(nn.Module):
    def __init__(self, img_shape: tuple, n_discriminators = 5, p_drop = 0.5, alpha=0.2, beta=0.1) -> None:
        ...
        self.discriminator = Discriminator(n_discriminators, self.img_shape, alpha, beta)
    ...
    def fit(self, data_loader: DataLoader, device):
        ...
        for batch_num, batch in enumerate(data_loader):
            ...
            if torch.count_nonzero(active) == 0:
                idx = torch.randint(high=len(dgz), size=(1,)).item()
                dgz = dgz[idx-1:idx]
            else:
                dgz = dgz[active]
    ...
    def generator_loss(self, dgz):
        d1_losses = torch.mean(-dgz[:, :, 0], dim=1)
        d2_losses = torch.mean(clamped_log(dgz[:, :, 1]), dim=1)
        losses = d1_losses + self.beta * d2_losses
